...
class OneFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        print("instances" in batched_inputs[0]) # 可以确定，eval的时候是没有instance在batched inputs里面的，不知道train的时候有没有
        print('############# batched_inputs from oneformer_model.py ##############')
        """
        Args:
    ...
    def panoptic_inference(self, mask_cls, mask_pred):
        ...
        print('##################### cur_classes and cur_masks from oneformer_model.py ########################')
        print(cur_classes)
        print(cur_masks)
        ...
        plt.axis('off')  # Remove axis
        plt.colorbar()  # Optional: adds a colorbar
        plt.savefig(os.path.join(output_dir, f'uncertainty_map.png'), bbox_inches='tight', pad_inches=0)
        plt.close()
        ...
        # Save the figure without borders and padding
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
        plt.savefig(os.path.join(output_dir, f'uncertainty_map_2.png'), bbox_inches='tight', pad_inches=0)
        plt.close()
        ...
        if cur_masks.shape[0] == 0:
            ...
            # numpy_array = cur_mask_ids.cpu().numpy().astype(int)
            # # Save the NumPy array to a CSV file
            # np.savetxt('/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Code/OneFormer_ACDC_eval_with_TTA/cur_mask_ids.csv', numpy_array, delimiter=',', fmt='%d')
            
            pre_class_map = cur_classes[cur_mask_ids]
            numpy_array2=pre_class_map.cpu().numpy().astype(int)
            np.savetxt('/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Code/OneFormer_for_ACDC_eval_error_map_and_feature/error_map/pre_class_map.csv', numpy_array2, delimiter=',', fmt='%d')
            

            # from PIL import Image
            # # Convert the result tensor to a numpy array
            # numpy_result = numpy_array2

            # # Create a color map (25 unique colors)
            # color_map = np.random.randint(0, 255, (25, 3), dtype=np.uint8)

            # # Map the numbers in the numpy_result to the corresponding colors
            # colored_image = color_map[numpy_result]

            # # Convert the numpy array to an image
            # image = Image.fromarray(colored_image)

            # # Save the image
            # image.save('/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Code/OneFormer_ACDC_eval_with_TTA/colored_tensor_image.png')
