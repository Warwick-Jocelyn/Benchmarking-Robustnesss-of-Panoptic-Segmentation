...
class OneFormerHead(nn.Module):
    ...
    def layers(self, features, tasks, mask=None):
        ...
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            ...
            for i, tensor in enumerate(multi_scale_features):
                ...
                print('############################multiscale_feature############################')

                print(res.shape)

                tensor_image = res.squeeze(0)

                print(tensor_image.shape)

                tensor_image.cuda()
                tensor_float = tensor_image.float()
                img = tensor_float[0]
                save_image(img, f'/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Backup/OneFormer_for_index/multi_scale_feature/multi_scale_feature_{count}.png')
                
                # if count == 2:
                #     # Step 1: Read the file path from the text file
                #     with open('/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Backup/OneFormer_for_index/input_path.txt', 'r') as file:
                #         old_path = file.readline().strip()  # Read the first line and strip any extra whitespace/newlines

                #     # Step 2: Extract the image file name from the path
                #     image_name = old_path.split('/')[-1]  # Split the path by '/' and take the last segment as the image name

                #     # Step 3: Construct the new file path
                #     new_path = '/home/wang3_y_WMGDS.WMG.WARWICK.AC.UK/Backup/OneFormer_for_index/multi_scale_feature'
                #     full_new_path = f'{new_path}/{image_name}'  # Append the image name to the new directory path

                #     # Modify the save_image call to use this new path
                #     save_image(img, full_new_path)
